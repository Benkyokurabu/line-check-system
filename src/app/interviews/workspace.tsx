...
export default function ParentInterviews({trial=false,livePreview=false}:{trial?:boolean;livePreview?:boolean}){
 ...
 {!trial&&(state.students.find(s=>s.id===studentId)?.teacher?<p>担任：{state.students.find(s=>s.id===studentId)?.teacher}先生<br/>{state.invitationOnly?'教室から案内された日程だけを表示しています。':'明日以降の空き日程を自動で表示しています。'}</p>:<p>担任の登録を確認できません。教室へLINEでご連絡ください。</p>)}
