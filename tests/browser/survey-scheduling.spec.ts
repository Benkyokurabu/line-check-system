...
test('面談日程は対応状況と独立し、日時・絞り込み・復帰時更新・取得失敗を表示する',async({page})=>{
 ...
 const handled=list.getByRole('listitem').filter({hasText:'日程確認1'});await expect(handled.getByRole('combobox',{name:'日程確認1の対応状況'})).toHaveValue('confirmed');await expect(handled.getByRole('link',{name:'日程確認1：未連絡・候補日時を選ぶ'})).toHaveAttribute('href',`/staff/interviews?tab=invitations&answer=${ids[0]}`);
 ...
 await page.getByRole('combobox',{name:'アンケートの面談状況',exact:true}).selectOption('invited');await expect(list.getByRole('listitem')).toHaveCount(1);
 ...
 await page.getByRole('combobox',{name:'アンケートの面談状況',exact:true}).selectOption('');await expect(list.locator('[data-status="unknown"]')).toHaveCount(3);await expect(list.getByText('未連絡',{exact:true})).toHaveCount(0);
});
...
test('未連絡から対象生徒のNotion候補日時へ直接進み、送信制限は維持する',async({page})=>{
 ...
 await page.setViewportSize({width:390,height:844});await page.goto('/');await page.getByRole('combobox',{name:'アンケートの担任'}).selectOption('工藤');await page.getByRole('link',{name:'日程確認1：未連絡・候補日時を選ぶ'}).click();
