...
test('student trial submits, sees approval from another screen, then cancels',async({page})=>{
  ...
  await expect(page.getByRole('img',{name:/本校自習室の配置図/})).toBeVisible();
  await expect(page.getByRole('button',{name:/14:55/})).toHaveCount(0);
  await page.getByRole('button',{name:'16:45–18:15 空きあり',exact:true}).click();
