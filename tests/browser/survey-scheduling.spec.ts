...
test('回答確認から面談終了までを一つの進捗として表示・絞り込みする',async({page})=>{
 ...
 const handled=list.getByRole('listitem').filter({hasText:'日程確認1'});await expect(handled.getByRole('combobox',{name:'日程確認1の対応状況'})).toHaveValue('confirmed');await expect(handled.getByRole('link',{name:'日程確認1：面談日程を案内'})).toHaveAttribute('href',`/staff/interviews?tab=invitations&answer=${ids[0]}`);
 await expect(list.getByText('日程調整中',{exact:true})).toBeVisible();await expect(list.getByText('日程確定',{exact:true})).toBeVisible();await expect(list.getByText('2026-09-30 16:00〜16:45')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
