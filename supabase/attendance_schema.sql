...
-- Human decisions are scoped to one attendance message.
...
-- Keep the candidate header and all editable rows in one database transaction.
-- This prevents a failed row insert from leaving the candidate with deleted rows.
...
create or replace function public.save_attendance_candidate_review(
  p_candidate_id uuid, p_candidate jsonb, p_items jsonb, p_actor text
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  update public.attendance_candidates
    set human_reviewed_at = now(), human_reviewed_by = coalesce(nullif(btrim(p_actor), ''), human_reviewed_by)
    where id = p_candidate_id
...
-- Only new audited initial replies use reply_kind=initial. Historical rows without
-- reply_kind are intentionally excluded so the existing duplicate history remains intact.
