...
基本の元データは、プロジェクト内の `01 ★クラス一覧表(2026)` フォルダにあるクラス一覧Excelです。

対象ファイル:

- ファイル名に `クラス一覧表` を含む
- 拡張子が `.xlsx`
- 正本フォルダが存在する場合は、プロジェクト直下にある旧コピーではなく正本フォルダを優先する
...
- ファイル名から学年を判定する
- 生徒番号・生徒名が空の行は取り込まない
- 担任が空の場合は `未設定` として取り込み、Notion同期時に登録済みの担任情報があれば補完する
...
Notion生徒情報に在塾生として登録されているが、クラス一覧Excelにまだ載っていない生徒は、`Notionのみ` として表示されます。

この生徒は選択して反映できます。反映すると `student_roster` に `source_file = Notion生徒情報DB` の行を作成します。これにより、LINE管理名・LINE表示名・Notion生徒情報を使った紐づけが先に可能になります。

後日、同じ学籍番号の生徒がクラス一覧Excelに載った場合は、通常のExcel同期で氏名表記・学年・担任・校舎・学校名・クラス所属をExcel側の正規情報へ更新します。

定期的なNotion同期では、Excel由来の生徒について氏名・学年・担任・校舎・学校名・性別を上書きせず、Notionで管理している授業形態だけを補完します。これにより、`両方` 所属や科目別教室を含むExcel側の校舎情報を維持します。

注意:

- Notionのみで登録した時点では、クラス所属は作成しません。
- LINE紐づけのキーは学籍番号です。後からExcelに同じ学籍番号で入れば、既存のLINE紐づけは維持されます。
- Excel側の表記がNotionと違う場合、Excel取込後はExcel側を正とします。
