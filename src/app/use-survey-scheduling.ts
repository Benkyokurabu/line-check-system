...
export type ScheduleStatus={status:'uncontacted'|'invited'|'confirmed'|'completed'|'unknown';detail:string;date?:string;start?:string;end?:string};
export function useSurveyScheduling(){
 ...
 const load=useCallback(async()=>{
  ...
  try{const r=await fetch('/api/interview-surveys/scheduling',{cache:'no-store',signal:AbortSignal.timeout(60000)});if(!r.ok)throw Error(r.status===401?'日程状況は職員ログイン後に確認できます。':'日程状況を取得できません。再取得してください。');const b=await r.json();
   if(!b.states||typeof b.states!=='object'||Array.isArray(b.states)||Object.values(b.states).some((s)=>!s||typeof s!=='object'||!('status' in s)||!['uncontacted','invited','confirmed','completed','unknown'].includes(String(s.status))))throw Error('日程状況の応答を確認できません。');
