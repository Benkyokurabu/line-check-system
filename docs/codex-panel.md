...
9月12日以降workerのheartbeatが停止し、自動起動タスクも存在せず依頼が受付済みのまま滞留していた。Windows PowerShellでUTF-8 BOMなしの日本語を誤解釈してインストーラーが構文エラーになる問題を、ASCIIのタスク説明へ変更して修正。監督スクリプトはNodeのstderrで停止しないよう修正した。`BentanCodexPanel`をログオン時起動で登録し稼働確認、実APIで送信→worker→回答完了を検証済み。PCのログオン中に動作する構成。

UIは接続状態・作業状態を短いラベルと色で表示し、依頼入力・場所の指定・履歴を整理。参考は[Linear Asks](https://linear.app/docs/linear-asks)。アンケート一覧の生徒検索と未確認フィルターは[Typeformの回答一覧](https://help.typeform.com/hc/en-us/articles/360029253732-Working-with-your-responses)を参考にした。
