...
資料確認画面には、見つかった原本資料と指導簿・選択したアンケート回答を資料ごとのチェックボックスで表示します。初期状態ではすべて選択し、チェックした資料だけをPDFに収録してOneDriveへ保存・印刷します。アンケート回答を外した場合はNotion原本の読込も行いません。選択が空なら作成できません。資料が確認後に更新・移動された場合は作成を止め、再確認を促します。
