...
test('生徒の絞り込み・選択保持・工藤限定の案内確認と送信',async({page})=>{
 ...
 await page.getByRole('checkbox',{name:/架空生徒/}).check();await expect(page.getByRole('button',{name:'担任の空き日程を取得'})).toBeDisabled();await page.getByRole('button',{name:'選択を解除',exact:true}).click();
 ...
 await page.getByRole('button',{name:'担任の空き日程を取得'}).click();await page.getByRole('checkbox',{name:'2030-01-03 の枠を選択'}).check();await page.getByLabel('回答期限（日本時間）').fill('2030-01-02T12:00');await page.getByRole('button',{name:'案内内容を確認'}).click();
