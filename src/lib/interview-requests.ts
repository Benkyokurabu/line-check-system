...
export type Slot={id:string;notion_page_id:string;data:Record<string,string>;version:number;published:boolean;notion_edited_at:string};
...
export function available(slot:Slot,state:InterviewState,teacher:unknown){
 return slot.published&&slot.data.date>=dayAfter(2)&&slot.data.date<=dayAfter(60)&&teacherKey(slot.data.teacher)===teacherKey(teacher)
...
export async function notionOffers(){
 ...
 const pages=await queryPages(bensukeRequest,BENSUKE_SOURCE,{and:[{property:'日時',date:{on_or_after:dayAfter(2)}},{property:'日時',date:{on_or_before:dayAfter(60)}},{or:[{property:'内容',multi_select:{contains:'本：予約可'}},{property:'内容',multi_select:{contains:'南：予約可'}}]}]});
...
export async function publishData(pageId:string,state:InterviewState){
 ...
 if(!a?.usable||!('date' in a)||a.date<dayAfter(2)||a.date>dayAfter(60))throw new InterviewError('2日後から60日先までの予約可を選んでください。');
