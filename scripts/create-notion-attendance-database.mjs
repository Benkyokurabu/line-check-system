...
const created = await notion("/databases", {
  ...
  body: JSON.stringify({
    ...
    initial_data_source: {
      ...
      properties: {
        "（旧）理由": { title: {} },
        "名前": { relation: { data_source_id: studentDataSourceId, type: "single_property", single_property: {} } },
        "学籍番号": { rich_text: {} },
        "選択": { select: { options: ["欠席", "遅刻", "早退"].map((name) => ({ name })) } },
        "理由等": { rich_text: {} },
        "日付": { date: {} },
        "授業": { select: { options: [] } },
        "科目": { rich_text: {} },
        "授業校舎": { select: { options: [{ name: "本校" }, { name: "南教室" }] } },
        "LINE原文": { rich_text: {} },
        "LINE受信日時": { date: {} },
        "確認者": { rich_text: {} },
        "確認日時": { date: {} },
