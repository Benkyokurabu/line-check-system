...
function printHelp() {
  console.log(`Usage:
  npm run export:line-manager-chats -- [options]
...
function exportExpression(limit) {
  return `async () => {
    ...
    let guard = 0;
    do {
      const url = new URL('/api/v2/bots/' + botId + '/chats', location.origin);
      ...
      if (!response.ok) throw new Error('chats API failed: ' + response.status);
      const data = await response.json();
      for (const item of data.list ?? []) {
        rows.push({
          line_user_id: item.profile?.userId ?? item.chatId ?? '',
          alias_name: item.profile?.nickname ?? '',
          friend_display_name: item.profile?.name ?? '',
          latest_text: item.latestEvent?.message?.text ?? '',
          last_received_at: item.lastReceivedAt ? new Date(item.lastReceivedAt).toISOString() : '',
          chat_exists: true,
        });
      }
      next = data.next || '';
...
async function main() {
  ...
  if (result.exceptionDetails) {
    throw new Error(result.exceptionDetails.text ?? "Failed to export chats.");
  }
  const rows = (result.result.value ?? []).filter((row) => row.line_user_id);
