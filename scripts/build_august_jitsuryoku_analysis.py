...
.panel {{ display:none; background:var(--panel); border:1px solid var(--line); border-top:0; padding:12px; overflow:visible; width:max-content; min-width:100%; }} .panel.active {{ display:block; }} .table-wrap {{ display:block; width:max-content; min-width:100%; overflow:visible; background:#fff; position:relative; }} .panel-toolbar {{ position:sticky; top:var(--panel-toolbar-top); z-index:30; background:var(--panel); border-bottom:1px solid var(--line); padding:0 0 8px; margin:0 0 10px; }} h2 {{ font-size:16px; margin:0 0 8px; }} .note {{ color:var(--muted); margin:0 0 10px; }}
table {{ border-collapse:separate; border-spacing:0; width:max-content; min-width:100%; }} th,td {{ border-right:1px solid #e3e8ef; border-bottom:1px solid #e3e8ef; padding:6px 7px; white-space:nowrap; text-align:left; }} th {{ position:sticky; top:var(--table-head-top); z-index:20; background:var(--head); font-weight:700; color:#2d3748; }} td:nth-child(n+7) {{ text-align:right; }} tbody tr:hover {{ background:#f8fbff; }}
...
@media (max-width:760px) {{ header {{ position:static; }} .top-controls,.panel-toolbar {{ position:static; }} .panel,.table-wrap {{ width:auto; min-width:100%; }} main {{ padding:12px; }} .metrics {{ grid-template-columns:repeat(2,minmax(120px,1fr)); }} input,select {{ width:100%; }} label {{ flex:1 1 140px; }} th {{ top:0; }} }}
</style>
</head>
<body>
