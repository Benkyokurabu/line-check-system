...
.panel {{ display:none; background:var(--panel); border:1px solid var(--line); border-top:0; padding:12px; overflow:visible; }} .panel.active {{ display:block; }} .table-wrap {{ max-width:100%; overflow-x:auto; overflow-y:visible; background:#fff; position:relative; }} .panel-toolbar {{ position:sticky; top:var(--panel-toolbar-top); z-index:30; background:var(--panel); border-bottom:1px solid var(--line); padding:0 0 8px; margin:0 0 10px; }} h2 {{ font-size:16px; margin:0 0 8px; }} .note {{ color:var(--muted); margin:0 0 10px; }}
table {{ border-collapse:separate; border-spacing:0; width:max-content; min-width:100%; }} th,td {{ border-right:1px solid #e3e8ef; border-bottom:1px solid #e3e8ef; padding:6px 7px; white-space:nowrap; text-align:left; }} th {{ position:sticky; top:var(--table-head-top); z-index:5; background:var(--head); font-weight:700; color:#2d3748; }} td:nth-child(n+7) {{ text-align:right; }} tbody tr:hover {{ background:#f8fbff; }}
.placement-table th:nth-child(-n+4),.placement-table td:nth-child(-n+4) {{ position:sticky; z-index:12; background-clip:padding-box; }} .placement-table th:nth-child(1),.placement-table td:nth-child(1) {{ left:0; min-width:52px; width:52px; }} .placement-table th:nth-child(2),.placement-table td:nth-child(2) {{ left:52px; min-width:48px; width:48px; }} .placement-table th:nth-child(3),.placement-table td:nth-child(3) {{ left:100px; min-width:86px; width:86px; }} .placement-table th:nth-child(4),.placement-table td:nth-child(4) {{ left:186px; min-width:142px; width:142px; }} .placement-table td:nth-child(-n+4) {{ background:#fff; }} .placement-table tbody tr:hover td:nth-child(-n+4) {{ background:#f8fbff; }} .placement-table th:nth-child(-n+4) {{ background:var(--head); z-index:18; }} .placement-table th:nth-child(4),.placement-table td:nth-child(4) {{ box-shadow:8px 0 10px -10px rgba(23,32,51,.45); }}
...
<section id="placement" class="panel active"><div class="panel-toolbar"><h2>クラス替え資料</h2><p class="note">変更案はこの画面上だけの作業用です。人数は校舎別に集計され、変更すると即時更新されます。列見出しをクリックすると各テスト・偏差値でソートできます。</p></div><div id="classCounts" class="class-counts"></div>{placement_html}</section>
...
<section id="dist" class="panel"><div class="panel-toolbar"><h2>点数分布</h2></div><div class="table-wrap"><table><thead><tr><th>学年</th><th>校舎</th><th>科目</th><th>人数</th><th>分布</th><th>80以上</th><th>60-79</th><th>40-59</th><th>39以下</th></tr></thead><tbody>{''.join(dist_rows)}</tbody></table></div></section>
</main>
