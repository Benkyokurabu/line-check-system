...
test("known student's enrolled lessons start selected and can be deselected", async ({ page }) => {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (url.pathname === "/api/attendance/candidates") return route.fulfill({ json: { candidates: [{
      id: "known-student", student_number: "known", student_roster: student,
      status: "pending", event_type: "absence", event_date: "2099-09-11",
      ai_summary: "体調不良", ai_confidence: 0.9,
      sender_profile: { display_name: "山田保護者", alias_names: [], account_names: [] },
      line_messages: { id: "known-message", line_user_id: "known-line", display_name: "山田保護者", text: "欠席します", received_at: "2099-09-10T09:00:00Z" },
      attendance_candidate_items: [{ id: "known-item", student_number: "known", event_type: "absence", event_date: "2099-09-11", lesson_id: null, suggested_subject: null, suggested_class_name: null, ai_summary: "体調不良", status: "pending" }],
      reply_messages: [],
    }] } });
    if (url.pathname === "/api/attendance/lessons") return route.fulfill({ json: { lessons: [
      ...
      { id: "science", label: "理科C", lesson_date: "2099-09-11", start_time: "20:00", campus: "本校", enrolled: false },
    ] } });
  ...
  const science = page.getByRole("button", { name: /理科C/ });
  await expect(math).toHaveAttribute("aria-pressed", "true");
  await expect(english).toHaveAttribute("aria-pressed", "true");
  await expect(math).toHaveCSS("background-color", "rgb(220, 252, 231)");
  await expect(science).toHaveAttribute("aria-pressed", "false");
  await expect(science).toHaveCSS("background-color", "rgb(255, 255, 255)");
  await math.click();
  await expect(math).toHaveAttribute("aria-pressed", "false");
  await expect(math).toHaveCSS("background-color", "rgb(255, 255, 255)");
});

test("student selection waits for lessons before saving the draft", async ({ page }) => {
  const student = { student_number: "known", student_name: "山田 花子", grade: "中2", campus: "本校", homeroom_teacher: "佐藤" };
  const writes: Array<{ items: Array<{ lesson_id: string | null }> }> = [];
  await page.route("**/api/**", async (route) => {
    const url = new URL(route.request().url());
    if (url.pathname === "/api/attendance/students") return route.fulfill({ json: { students: [student] } });
    if (url.pathname === "/api/attendance/candidates" && route.request().method() === "GET") return route.fulfill({ json: { candidates: [{
      id: "student-selection", student_number: null, student_roster: null,
      student_selection_required: true, student_suggestions: [student],
      status: "pending", event_type: "absence", event_date: "2099-09-11",
      ai_summary: "体調不良", ai_confidence: 0.5,
      sender_profile: { display_name: "山田保護者", alias_names: [], account_names: [] },
      line_messages: { id: "student-selection-message", line_user_id: "known-line", display_name: "山田保護者", text: "欠席します", received_at: "2099-09-10T09:00:00Z" },
      attendance_candidate_items: [{ id: "student-selection-item", student_number: null, event_type: "absence", event_date: "2099-09-11", lesson_id: null, suggested_subject: null, suggested_class_name: null, ai_summary: "体調不良", status: "pending" }],
      reply_messages: [],
    }] } });
    if (url.pathname === "/api/attendance/lessons") {
      if (url.searchParams.get("student_number") === "known") {
        await new Promise((resolve) => setTimeout(resolve, 1200));
        return route.fulfill({ json: { lessons: [{ id: "math", label: "数学A", lesson_date: "2099-09-11", start_time: "17:00", campus: "本校", enrolled: true }] } });
      }
      return route.fulfill({ json: { lessons: [] } });
    }
    if (url.pathname === "/api/attendance/candidates/student-selection" && route.request().method() === "PATCH") {
  ...
  expect(writes[0].items.map((item) => item.lesson_id)).toEqual(["math"]);
});
