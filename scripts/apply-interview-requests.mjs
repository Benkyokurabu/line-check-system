...
try{
 ...
 const backupPath=path.join(directory,`database.before_parent_requests_${new Date().toISOString().replace(/[:.]/g,'-')}.json`);
 fs.writeFileSync(backupPath,JSON.stringify(backup,null,2));
 const sql=fs.readFileSync(new URL('../supabase/interview_requests_20260916.sql',import.meta.url),'utf8').replace(/^begin;\s*/,'').replace(/commit;\s*$/,'');
 ...
 const grants=(await db.query("select has_function_privilege('anon','interview_parent_submit(text,uuid,uuid,uuid[],text)','execute') anon,has_function_privilege('authenticated','interview_review_request(uuid,uuid,uuid,uuid,integer,text,uuid,text,jsonb,text,text)','execute') authenticated")).rows[0];
