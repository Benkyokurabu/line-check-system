...
test('金城先生専用の60分・50分枠と授業条件を表示する',async({page})=>{
 ...
 await expect(page.locator('.slot').filter({hasText:'⑩ 21:25〜22:15'}).getByText('候補',{exact:true})).toBeVisible();
 await expect(page.getByText('⑪ 22:05〜終了未定')).toHaveCount(0);
 await page.getByRole('checkbox',{name:'20:25〜21:55の授業あり'}).check();
 await expect(page.locator('#summary')).toContainText('9枠');
 await expect(page.locator('.slot').filter({hasText:'⑩ 21:25〜22:15'}).getByText('対象外',{exact:true})).toBeVisible();
 await expect(page.locator('.slot').filter({hasText:'⑪ 22:05〜終了未定'}).getByText('候補',{exact:true})).toBeVisible();
 await page.getByRole('checkbox',{name:'14:55〜16:25の授業あり'}).check();
 await expect(page.locator('#summary')).toContainText('7枠');
 await page.getByRole('checkbox',{name:'20:25〜21:55の授業あり'}).uncheck();
 await expect(page.locator('#summary')).toContainText('8枠');
 await expect(page.getByText('⑪ 22:05〜終了未定')).toHaveCount(0);
 await expect(page.locator('.slot').filter({hasText:'⑩ 21:25〜22:15'}).getByText('候補',{exact:true})).toBeVisible();
 expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);
});
