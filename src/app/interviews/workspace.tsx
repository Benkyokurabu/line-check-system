...
export function describe(d:Time){return `${new Intl.DateTimeFormat('ja-JP',{month:'long',day:'numeric',weekday:'short',timeZone:'Asia/Tokyo'}).format(new Date(d.date+'T12:00:00+09:00'))} ${d.start}〜${d.end||'（終了時刻なし）'}`;}
export default function ParentInterviews({trial=false,livePreview=false}:{trial?:boolean;livePreview?:boolean}){
 ...
 return <main className={styles.screen}><h1>面談予約</h1><p>{trial?'オンライン・45分':'オンライン面談・時間は各枠に表示'}</p>{trial&&<p className={styles.notice}>生徒役の検証用です。架空の日程で申請を試せます。実予約・Notion登録・LINE通知は行いません。</p>}{livePreview&&<p className={styles.notice}>本番と同じ申請・先生承認・Notion登録を確認する画面です。確認用生徒の予約として登録され、実際の面談ではありません。</p>}
 ...
 {review&&state&&<FlowDialog label="予約希望の確認" onBack={()=>setReview(false)} blocked={frozen} notice={notice}><h2>この希望日程で送信しますか？</h2><p>{state.students.find(s=>s.id===studentId)?.name}さん ／ オンライン面談</p><ol>{choices.map(c=><li key={c.id}>{describe(c)}</li>)}</ol>{note&&<p>{note}</p>}<p>送信後は承認待ちになります。</p><button className={styles.primary} disabled={frozen||!valid} aria-live="polite" onClick={()=>void send({operationKey:crypto.randomUUID(),action:'submit',studentId,invitationId:invitation?.id,invitationVersion:invitation?.version,choices:choices.map(c=>c.id),note})}>{busy?'送信中…':'予約希望を送信する'}</button></FlowDialog>}
 </main>;
}
